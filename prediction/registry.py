...
# --------------------------------------------------------------------------- #
# Model groups (UNIFIED handoff §11.6)                                        #
# --------------------------------------------------------------------------- #
...
# Monkey-patch methods onto ModelRegistry
...
def save_group(
    ...
    *,
    ...
    for role, mid in comps.items():
        ...
        # Ensure component metadata is loadable
...
def load_group(self, group_id: str) -> ModelGroupMetadata:
    ...
    try:
        ...
        raise RegistryError(f"unreadable group {group_id!r}: {exc}") from exc
    meta = ModelGroupMetadata.from_dict(data)
    self.validate_group(meta)
    return meta


def set_group_status(self, group_id: str, status: str, note: str = "") -> ModelGroupMetadata:
    if status not in STATUSES:
        raise RegistryError(f"unknown status {status!r}")
...
def validate_group(
    ...
    *,
    ...
    if load_mode is not None:
        from prediction.deployment import assert_mode_permission, DeploymentError
        try:
...
ModelRegistry._groups_dir = _groups_dir
ModelRegistry._group_meta_path = _group_meta_path
ModelRegistry.save_group = save_group
ModelRegistry.load_group = load_group
ModelRegistry.set_group_status = set_group_status
ModelRegistry.validate_group = validate_group
ModelRegistry.list_groups = list_groups
