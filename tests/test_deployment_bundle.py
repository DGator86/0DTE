...
================================
PR E — DeploymentBundle, complete hashing, strict modes, account isolation.
"""
...
def test_valid_bundle_loads(tmp_path):
    ...
    assert loaded.bundle_schema_version == "1"
    assert loaded.configuration_hash == ch
    assert configuration_hash(loaded.to_dict()) == ch
    assert loaded.prediction_model_group_id == "g1"
    # Dual keys on disk for legacy pointer readers.
    assert loaded.to_dict()["prediction_model_group"] == "g1"
...
def test_hash_changes_when_api_schema_version_changes():
    a = _bundle(api_schema_version="live.v1")
    b = _bundle(api_schema_version="live.v2")
    assert configuration_hash(a.to_dict()) != configuration_hash(b.to_dict())
...
def test_candidate_requires_distinct_account():
    b = _bundle(mode="candidate", candidate_account_id=None)
    with pytest.raises(DeploymentError, match="candidate_account_id"):
        validate_deployment_bundle(b)
    same = _bundle(mode="candidate", candidate_account_id="legacy",
                   reference_account_id="legacy")
    with pytest.raises(DeploymentError, match="differ"):
        validate_deployment_bundle(same)
    ok = _bundle(mode="candidate", candidate_account_id="candidate")
    validate_deployment_bundle(ok)


def test_candidate_cannot_use_heuristic_flag():
    b = _bundle(mode="candidate", candidate_account_id="cand")
