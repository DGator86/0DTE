...
| `matrix_universe.py` | Hierarchical Markov universe generator: per-variable chains → regimes → market archetypes; combinatoric seeded lattice (72 cells) with coverage tracking and generation-over-generation evolution toward weak archetypes. Implements the DataFeed protocol. | `MarkovWorldFeed`, `UniverseCatalog`, `UniverseSpec`, `merge_coverage` | Validated |
| `integrations/spy_der/` | Thin SPY-DER boundary: Market/Outcome packet publishers, HTTP decision client (`:8787`), dashboard reader (`/var/lib/spy-der/*`), experience + synthetic providers, mechanical champion apply. AI/Dojo/learning live in SPY-DER. See `docs/migrations/PHASE5_AI_OWNERSHIP_REMOVAL.md`. | `DecisionClient`, `publish_market_packet`, `MarketExperienceProvider` | Validated |
| `regime_calibration.py` | Regime-label bridge + empirical transition calibration for the universe simulator: session-relative feature labeler (observable features → simulator regime/archetype), empirical transition estimator (round-trip validated), and `calibrate_from_feed` producing overrides `MarkovWorldFeed` consumes. Labeling latent regimes is approximate (documented). | `calibrate_from_feed`, `estimate_transitions`, `label_regime`, `label_archetype`, `labeler_accuracy` | Validated |
