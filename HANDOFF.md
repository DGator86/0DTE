...
| `dojo.py` | Matrix-style training run: recorded-tape walk-forward + one adaptive-learning cycle + universe sparring; persists a dojo report (`validation_reports`, `report_type='dojo'`) rendered by the dashboard Dojo tab. Per-session directional decomposition; optional `--calibrate-from-recorded`. See `docs/dojo.md`. | `run_dojo`, `DojoConfig`, CLI | Validated |
